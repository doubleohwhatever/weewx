...
if __name__ == "__main__":
    ...
    #with WS_UMB_dummy() as umb:
        mydict = {}
        value, status = umb.onlineDataQuery(sys.argv[3], sys.argv[1], int(sys.argv[2]))
        if status == 0:
            mydict[sys.argv[3]] = value
        else:
            sys.stderr.write("On channel " + str(sys.argv[3]) + " got bad " + umb.checkStatus(status) + "\n")
