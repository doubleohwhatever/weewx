...
#            cmd = "%sxxx" % self.crc_prefix
    ...
    OBSERVATIONS = {
        ...
        # aR4: analog sensors
        'Tr': 'analog_temperature',
        'Ra': 'analog_rain',
        'Sl': 'analog_ultrasonic_level',
        'Sr': 'analog_solar_radiation',
...
class WXT5x0Driver(weewx.drivers.AbstractDevice):
    ...
    def __init__(self, **stn_dict):
        ...
        self._poll_interval = int(stn_dict.get('poll_interval', 3))
            ...
            for cnt in range(self._max_tries):
                try:
                    ...
                    try:
                        ...
                        pass
                    try:
                        packet['UV'] = (float(subprocess.check_output(['/root/WS_UMB_SIMPLE_SINGLE.py','2','7','902']).rstrip()) * 2) #DEVICE-ID:2 CLASS:7 CHANNEL:902 (UV act) - From Lufft WS10
                    except:
                        pass
                    try:
                        packet['luminosity'] = float(subprocess.check_output(['/root/WS_UMB_SIMPLE_SINGLE.py','2','7','903']).rstrip()) #DEVICE-ID:2 CLASS:7 CHANNEL:903 (klx act) - From Lufft WS10
                    except:
                        pass
                    try:
